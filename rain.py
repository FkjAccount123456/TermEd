...
priority = {
    TokenType.MUL: 100, TokenType.DIV: 100, TokenType.MOD: 100,
    TokenType.ADD: 99, TokenType.SUB: 99,
    TokenType.LSH: 98, TokenType.RSH: 98,
    TokenType.EQ: 97, TokenType.NE: 97,
    TokenType.GT: 96, TokenType.GE: 96, TokenType.LT: 96, TokenType.LE: 96,
    TokenType.BITAND: 95, TokenType.BITOR: 94, TokenType.XOR: 93,
    TokenType.AND: 92, TokenType.OR: 91,
}
...
def tokenize(code: str) -> list[Token]:
    ...
    while pos < len(code):
        ...
        if pos >= len(code):
            ...
            if not pair_begins or pair_begins[-1] not in (TokenType.LBRACE, TokenType.LPAREN, TokenType.LSQBR):
                res.append(Token(TokenType.EOL))
            pos += 1
            ...
            if ident in keywords:
                ...
                if res[-1].type in (TokenType.IF, TokenType.WHILE, TokenType.FOR, TokenType.FUNC):
                    pair_begins.append(res[-1].type)
                elif res[-1].type == TokenType.END:
                    if pair_begins and pair_begins[-1] in (TokenType.IF, TokenType.WHILE, TokenType.FOR, TokenType.FUNC):
                        pair_begins.pop()
                    else:
            ...
            while pos < len(code) and code[pos] != quote:
                if code[pos] == '\\':
                    ...
                    if pos >= len(code):
                        ...
                        raise RainSyntaxError(
                            f'Invalid escape sequence: \\{code[pos]}')
                else:
                    string += code[pos]
                    pos += 1
            if pos >= len(code):
    ...
    def compile_expr(expr):
        match expr:
            ...
            case 'func', args, body:
                ...
                output.append(('func', len(output) + 1, "<lambda>",
                              len(args), len(fn_vars)))
                output.append(None)
                pos = len(output) - 1
                scope.append({})
            ...
            case 'require', path:
                ...
                prog = parse(tokens)
                vars = hoist(prog)
                output.append(('func', len(output) + 1, f'<module {path}>', 0, len(vars)))
                output.append(None)
                pos = len(output) - 1
                scope.append({})
    ...
    def compile_stmt(stmt):
        match stmt:
            ...
            case 'func', name, args, body:
                fn_vars = args + hoist(body)
                output.append(('func', len(output) + 2, name,
                              len(args), len(fn_vars)))
                output.append(('store', *find_var(name)))
                output.append(None)
                pos = len(output) - 1
...
def run(prog: list[tuple], reserve_cnt: int,
        pc: int = 0, scope: list[list] = None):
    stack = []
    pc_stack = []
    scope_stack = []
    if scope is None:
